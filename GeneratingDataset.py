...
class BytePairEncoding(Vocabulary):
  ...
  def __init__(self, vocab_file, bpe_file, seq_postfix=None, unknown_label="UNK"):
    ...
    bpe_file_first_line = open(bpe_file, "r", encoding='utf-8').readline()
...
class LibriSpeechCorpus(CachedDataset2):
  ...
  def _open_audio_file(self, seq_idx):
    ...
    if self.prefix=="train":
        audio_fn = "%(sd)s/%(sp)i-%(ch)i-%(i)04i.wav" % {
          "sd": subdir, "sp": speaker_id, "ch": chapter_id, "i": seq_id}
    if self.prefix=="dev":
        audio_fn = "%(sd)s/%(sp)i-%(ch)i-%(i)04i.wav" % {
          "sd": subdir, "sp": speaker_id, "ch": chapter_id, "i": seq_id}
...
class CernEULibriUnescoUnogWipoCorpus_with_domain_tags_op(CachedDataset2):
  """
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(CernEULibriUnescoUnogWipoCorpus_with_domain_tags_op, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
  ...
  def _collect_trans(self):
    ...
    for subdir in glob("%s/%s*" % (self.path, self.prefix)):
      ...
      subdir = os.path.basename(subdir)  # e.g. "train-clean-100"
      if self.prefix=="train":
        fn_list = glob("%s/%s/*/*.trans.txt.tagged" % (self.path, subdir)) + [self.path+'/'+subdir+'/libri/train/wav/libri_train.trans.txt.tagged']
      elif self.prefix=="dev":
        fn_list = glob("%s/%s/*/*.trans.txt.tagged" % (self.path, subdir))
        #fn_list = [self.path + '/' + subdir + '/wipo_val.trans.txt']
      elif self.prefix=="test":
        fn_list = [self.path + '/' + subdir + '/wipo_test.trans.txt.tagged']
      elif self.prefix=="demo":
        fn_list = [self.path + '/' + subdir + '/demo.trans.txt.tagged']
      for fn in fn_list:
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(CernEULibriUnescoUnogWipoCorpus_with_domain_tags_op, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
...
class CernEULibriUnescoUnogWipoCorpus_with_domain_tags_ip_and_op(CachedDataset2):
  """
  LibriSpeech + WIPO + EUParl. + CERN + UNESCO + UNOG http://www.openslr.org/12/
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(CernEULibriUnescoUnogWipoCorpus_with_domain_tags_ip_and_op, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(CernEULibriUnescoUnogWipoCorpus_with_domain_tags_ip_and_op, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
    self._audio_random.seed(self._fixed_random_seed or epoch or 1)
    if seq_list is not None:
  ...
  def _collect_single_seq(self, seq_idx):
    ...
    subdir, seq_name = self._reference_seq_order[self._get_ref_seq_idx(seq_idx)]
    audio_fn = "%(sn)s.wav" % {"sn": seq_name}
    if "VODChapter" in audio_fn: #TODO COME HERE
...
class ruOpenSttCorpus(CachedDataset2):
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(ruOpenSttCorpus, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
    ...
    assert prefix.split("-")[0] in ["test", "train", "dev", "demo"]
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(ruOpenSttCorpus, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
    self._audio_random.seed(self._fixed_random_seed or epoch or 1)
    if seq_list is not None:
  ...
  def _open_audio_file(self, seq_idx):
    ...
    audio_fn = "%s/%s" % (subdir, audio_fn)
    audio_fn = "%s/%s" % (self.path, audio_fn)
    if not os.path.exists(audio_fn.encode('utf-8')): #filenames with utf-8:
        audio_fn = "%(sn)s.opus" % {"sn": seq_name}
        audio_fn = "%s/%s" % (subdir, audio_fn)
        audio_fn = "%s/%s" % (self.path, audio_fn)
    assert os.path.exists(audio_fn.encode('utf-8')) #filenames with utf-8
    return open(audio_fn.encode('utf-8'), "rb")
...
#French
class SpecAugFrCorpus(CachedDataset2):
  """
  EUParl-ST + ILO + COMMON VOICE
  ...
  def __init__(self, path, prefix, audio,
               ...
               **kwargs):
    """
    :param str path: dir, should contain 
    :param str prefix: "train", "dev", "test", "dev-clean", "dev-other", ...
    :param str|list[str]|None orth_post_process: :func:`get_post_processor_function`, applied on orth
    :param str|None targets: "bpe" or "chars" currently, if `None`, then "bpe"
    :param dict[str] audio: options for :class:`ExtractAudioFeatures`
    ...
    if not name:
      name = "prefix:" + prefix
    super(SpecAugFrCorpus, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
    ...
    assert prefix.split("-")[0] in ["train", "dev", "demo", "test", "dev_wipo", "dev_unesco", "dev_unog", "dev_cern", "test_wipo", "test_unesco", "test_unog", "test_cern, test_ilo"]
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(SpecAugFrCorpus, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
    self._audio_random.seed(self._fixed_random_seed or epoch or 1)
  ...
  def _collect_trans(self):
    ...
    for subdir in glob("%s/%s*" % (self.path, self.prefix)):
      ...
      print("subdir->",subdir)
      if self.prefix=="train":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged)) 
      elif self.prefix=="dev":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="demo":
        fn_list = [self.path + '/' + subdir + '/demo.trans.txt']
      elif self.prefix=="dev-cern":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="dev-unesco":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="dev-unog":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="dev-ilo":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="dev-wipo":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="test-cern":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="test-unesco":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="test-unog":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="test-wipo":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
      elif self.prefix=="test-ilo":
        fn_list = glob("%s/%s/*trans.txt%s" % (self.path, subdir, tagged))
  ...
  def _open_audio_file(self, seq_idx):
    ...
    if self.prefix=="dev" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-cern" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-unesco" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-unog" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-wipo" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-ilo" and "dev" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-ilo" and "test" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-cern" and "test" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-wipo" and "test" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-unog" and "test" in subdir:
        audio_fn = "%s/%s" % (subdir, audio_fn)
...
class SpecAugCernEULibriUnescoUnogWipoCorpus(CachedDataset2):
  """
  LibriSpeech + WIPO + EUParl. + CERN + UNESCO + UNOG http://www.openslr.org/12/
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(SpecAugCernEULibriUnescoUnogWipoCorpus, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(SpecAugCernEULibriUnescoUnogWipoCorpus, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
    self._audio_random.seed(self._fixed_random_seed or epoch or 1)
...
class CernEULibriUnescoUnogWipoCorpus(CachedDataset2):
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(CernEULibriUnescoUnogWipoCorpus, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import Util
    self.path = path
    ...
    assert prefix.split("-")[0] in ["test", "train", "dev", "demo", "dev_wipo", "dev_unesco", "dev_unog", "dev_cern", "test_wipo", "test_unesco", "test_unog", "test_cern"]
  ...
  def _collect_trans(self):
    ...
    for subdir in glob("%s/%s*" % (self.path, self.prefix)):
      ...
      elif self.prefix=="dev_ilo":
        fn_list = [self.path + '/dev_ilo' + '/ilo_dev.trans.txt']
      elif self.prefix=="dev-ilo":
        fn_list = [self.path + '/dev-ilo' + '/ilo_dev.trans.txt']
      elif self.prefix=="test_cern":
        fn_list = [self.path + '/test_cern' + '/cern_test.trans.txt']
      elif self.prefix=="test-cern":
        fn_list = [self.path + '/test-cern' + '/cern_test.trans.txt']
      elif self.prefix=="test_unesco":
        fn_list = [self.path + '/test_unesco' + '/unesco_test.trans.txt']
      elif self.prefix=="test-unesco":
        fn_list = [self.path + '/test-unesco' + '/unesco_test.trans.txt']
      elif self.prefix=="test_unog":
        fn_list = [self.path + '/test_unog' + '/unog_test.trans.txt']
      elif self.prefix=="test-unog":
        fn_list = [self.path + '/test-unog' + '/unog_test.trans.txt']
      elif self.prefix=="test_wipo":
        fn_list = [self.path + '/test_wipo' + '/wipo_test.trans.txt']
      elif self.prefix=="test-wipo":
        fn_list = [self.path + '/test-wipo' + '/wipo_test.trans.txt']
      elif self.prefix=="test-ilo":
        fn_list = [self.path + '/test-ilo' + '/ilo_test.trans.txt']
      for fn in fn_list:
        subsubdir = os.path.basename(os.path.dirname(fn))
        for l in open(fn, encoding='utf-8').read().splitlines():
          #utf8stdout = open(1, 'w', encoding='utf-8', closefd=False) # fd 1 is stdout
  ...
  def _open_audio_file(self, seq_idx):
    ...
    if self.prefix=="dev_wipo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev_ilo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="dev-ilo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    ...
    if self.prefix=="test_wipo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-wipo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    if self.prefix=="test-ilo":
      audio_fn = "%s/%s" % (subdir, audio_fn)
    audio_fn = "%s/%s" % (self.path, audio_fn)
    assert os.path.exists(audio_fn.encode('utf-8')) #filenames with utf-8
    return open(audio_fn.encode('utf-8'), "rb")
...
class CernCorpus(CachedDataset2):
  """
  Cern Corpus
  ...
  def __init__(self, path, prefix, audio,
    ...
    if not name:
      name = "prefix:" + prefix
    super(CernCorpus, self).__init__(name=name, **kwargs)
    import os
    from glob import glob
    import zipfile
    import Util
  ...
  def init_seq_order(self, epoch=None, seq_list=None):
    ...
    """
    import Util
    super(CernCorpus, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    if not epoch:
      epoch = 1
    self._audio_random.seed(self._fixed_random_seed or epoch or 1)
    if seq_list is not None:
